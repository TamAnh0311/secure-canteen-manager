...
export class MenuController {
  ...
  // The scanner receives only stable menu codes and display snapshots; UUIDs and
  // roster data never cross this admin-authorized catalogue boundary.
  ...
  // template on the single threshold_config row. The save stamps roiGeneratedAt, which
  // permanently locks reorder/rename/hard-delete so printed checkboxes stay bound to items.
  @Post('form')
  @Roles(OperatorRole.ADMIN)
  async generateForm(@Body('mode') requestedMode?: OmrFormMode): Promise<{ pdfBase64: string }> {
    ...
    // Reject before printing if any code would overflow the form's fixed-width code
    // boxes — a sheet that cannot represent its own codes would mis-decode dishes.
