...
import type { KioskMenuItem, CanteenPrisonerView } from '@/lib/types';
import type { CanteenBankTransfer, CanteenOrderResult } from '@/lib/api/kiosk';

// Both canteen reads/writes go through this namespace; mock it so no fetch fires.
vi.mock('@/lib/api', () => ({
  canteen: { getPrisonerView: vi.fn(), placeOrder: vi.fn() },
}));

const getPrisonerView = vi.mocked(canteen.getPrisonerView);
const placeOrder = vi.mocked(canteen.placeOrder);

const MENU: KioskMenuItem[] = [{ id: 'i1', name: 'Cơm', price: 15_000, category: 'food' }];

const VIEW: CanteenPrisonerView = { name: 'NGUYEN VAN A', prisonId: 'P0001', zone: 'A', cell: 'A-01', menu: MENU, bankEnabled: true, purchaseLimits: { food: { enabled: false, amount: null }, essential: { enabled: false, amount: null } } };

const TRANSFER: CanteenBankTransfer = {
...
describe('CanteenPage — post-order branching', () => {
  ...
  it('bank order with a transfer → renders the offline VietQR screen', async () => {
    const result: CanteenOrderResult = {
      orderId: 'o',
...
describe('CanteenPage — idle reset on the bank-QR screen', () => {
