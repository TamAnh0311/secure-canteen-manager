...
/**
 * Full-screen standalone kiosk for relatives to browse the shared menu and place
 * an order for a prisoner. No authentication. No AppShell. No operator chrome.
 * Route: /kiosk (top-level public, outside ProtectedRoute).
 */
export function KioskPage() {
  ...
  const [prisonerView, setPrisonerView] = useState<KioskPrisonerView | null>(null);
  const [lookupError, setLookupError] = useState<string | null>(null);
  const [confirmationCode, setConfirmationCode] = useState('');
  const [bankTransfer, setBankTransfer] = useState<KioskBankTransfer | null>(null);
  ...
  async function handleSubmit(prisonId: string) {
    ...
    try {
      const view = await kiosk.getPrisonerView(prisonId);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-start px-4 py-8 gap-6 max-w-2xl mx-auto w-full">
        {/* ── Entry screen ── */}
        {screen === 'entry' && (
          <>
